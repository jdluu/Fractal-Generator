...
let zoomLevel = 1;
let centerX = -0.5; // Adjust this value to center the Mandelbrot set horizontally
let centerY = 0; // Adjust this if needed for vertical centering
...
export function updateFractal() {
    ...
    console.log('Updating fractal:', { fractalType, r, g, b, iterations, width: canvas.width, height: canvas.height, zoomLevel, centerX, centerY });
...
function updateFractalInfo() {
    ...
    document.getElementById('fractal-description').innerHTML = `<h3>${info.title}</h3><p>${info.description}</p>`;
}
...
function resetView() {
    zoomLevel = 1;
    centerX = -0.5; // Reset to the initial centered position
    centerY = 0;
    updateFractal();
}
